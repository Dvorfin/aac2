...
if __name__ == "__main__":
    ...
    # Исходные данные
    data = [1.06, 1.1466, 1.0631, 1.0764, 1.0476, 1.0181, 1.0701]

    # Построение графика
    plt.figure(figsize=(10, 6))
    bars = plt.bar(range(len(data)), data, color='skyblue', edgecolor='black')

    # Изменение диапазона оси Y
    plt.ylim(min(data) - 0.1, max(data) + 0.1)
   # plt.ylim(0.5, max(data) + 0.1)  # Нижняя граница: 0.5, верхняя граница: немного выше максимального значения

    # Настройка заголовков и осей
    plt.title('Наглядное сравнение данных (ось Y начинается с 0.5)', fontweight='bold')
    plt.xlabel('Измерение', fontweight='bold')
    ...
    plt.xticks(range(len(data)), [f"Изм. {i + 1}" for i in range(len(data))], rotation=45)
